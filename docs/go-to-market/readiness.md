...
- **Версии разъезжаются.** `Version.swift:2` → `1.0.0`, `release.sh:5` → `1.0.0`,
  ...
  `Dikto-Pro-1.0.0.dmg`.
- **`dist-pro-ru/` не в `.gitignore`.** ✅ Исправлено после аудита. Корневой
  `.gitignore` содержал `dist` — паттерн без слэша матчит только точное имя,
  поэтому `dist-pro-ru/` пролезал. Прогон релиза оставлял в рабочем дереве
  885 МБ артефактов (DMG 429 МБ, `Dikto Pro.app`, `xcodebuild.log`), которые
  легко случайно закоммитить. Теперь игнорируются `dist/` и `dist-*/`.
...
| B23 | ~~`dist-pro-ru/` не в `.gitignore` (885 МБ артефактов в рабочем дереве)~~ — ✅ исправлено после аудита | P2 | Было: корневой `.gitignore` содержал `dist`, что не покрывало `dist-pro-ru/`. Стало: `dist/` + `dist-*/` | 5 мин | — |
