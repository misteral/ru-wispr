...
SPINNER_FRAMES=("⠋" "⠙" "⠹" "⠸" "⠼" "⠴" "⠦" "⠧" "⠇" "⠏")
SPIN_PID=""
LOG=$(mktemp /tmp/dikto-install.XXXXXX)
APP_PID=""
...
# ── Header ────────────────────────────────────────────────────────────
printf "\n"
printf "  ${BOLD}dikto${NC} ${DIM}— local voice dictation for macOS${NC}\n"
printf "  ${DIM}────────────────────────────────────────────${NC}\n"
...
if [[ "$(uname -m)" != "arm64" ]]; then
    ...
    die "dikto uses Metal GPU acceleration which is not available on Intel Macs."
fi
ok "Apple Silicon"
...
# ── Step 1: Clean up ─────────────────────────────────────────────────
if brew list dikto &>/dev/null || [ -d ~/Applications/Dikto.app ]; then
    step "Removing previous installation"
    start_spin "Cleaning up..."

    brew services stop dikto </dev/null >/dev/null 2>&1 || true
    brew uninstall --force dikto </dev/null >/dev/null 2>&1 || true
    brew untap misteral/dikto </dev/null >/dev/null 2>&1 || true
    tccutil reset Accessibility co.itbeaver.dikto </dev/null >/dev/null 2>&1 || true
    tccutil reset Microphone co.itbeaver.dikto </dev/null >/dev/null 2>&1 || true
...
start_spin "Tapping misteral/dikto..."
TAP_OUT=$(brew tap misteral/dikto --force </dev/null 2>&1) || {
    stop_spin
    fail "Failed to tap misteral/dikto"
    info "$TAP_OUT"
    die "Make sure git is installed."
}
stop_spin
ok "Tapped ${DIM}misteral/dikto${NC}"

TAP_DIR="$(brew --repository misteral/dikto 2>/dev/null)"
if [ -n "$VERSION" ]; then
    ...
    git -C "$TAP_DIR" checkout "$FORMULA_COMMIT" -- dikto.rb 2>/dev/null
fi

start_spin "Installing dikto${VERSION:+ v$VERSION}..."
brew install dikto </dev/null >/dev/null 2>&1 || true
brew reinstall dikto </dev/null >/dev/null 2>&1 || true
if [ -n "$VERSION" ]; then
    git -C "$TAP_DIR" checkout -- . 2>/dev/null || true
fi
stop_spin

BREW_PREFIX="$(brew --prefix dikto 2>/dev/null)"
CELLAR_BIN="${BREW_PREFIX}/Dikto.app/Contents/MacOS/dikto"

if [ ! -x "$CELLAR_BIN" ]; then
    die "Installation failed — binary not found. Run 'brew install dikto' manually."
fi
ok "Installed"

mkdir -p ~/Applications
rm -rf ~/Applications/Dikto.app
ln -sf "${BREW_PREFIX}/Dikto.app" ~/Applications/Dikto.app
APP_BIN=~/Applications/Dikto.app/Contents/MacOS/dikto
...
if wait_for_log "Accessibility: granted" 5; then
    ...
    info "System Settings will open — find ${BOLD}Dikto${NC} and toggle it ${BOLD}ON${NC}.\n"
...
start_spin "Starting..."
brew services start dikto </dev/null >/dev/null 2>&1 || true
stop_spin

sleep 1
if brew services list 2>/dev/null | grep -q "dikto.*started"; then
    ok "Running as background service"
else
    ok "Service registered"
    info "If not running, start manually: brew services start dikto"
fi

# ── Done ──────────────────────────────────────────────────────────────
hotkey=$(grep "^Hotkey:" "$LOG" 2>/dev/null | tail -1 | sed 's/^Hotkey: //')
model=$(grep "^Model:" "$LOG" 2>/dev/null | tail -1 | sed 's/^Model: //')
version=$(grep "^dikto v" "$LOG" 2>/dev/null | tail -1)
