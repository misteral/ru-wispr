...
class HotkeyManager {
    ...
    private func handleEvent(_ event: NSEvent) {
        if isModifierOnlyKey(keyCode) {
            ...
            // Determine if the modifier is now pressed or released
            let isDown: Bool
            switch keyCode {
            case 54, 55: isDown = event.modifierFlags.contains(.command)
            ...
            if isDown && !modifierPressed {
                modifierPressed = true
                onKeyDown?()
            } else if !isDown && modifierPressed {
